...
assert(rankingBoard.customerDecisionSummary?.lines?.some((line) => line.includes("可小仓复核") && buyActionCodes.some((code) => line.includes(code))), "customer decision summary must translate buy-review rankings into a customer-readable buy line");
assert(rankingBoard.customerDecisionSummary?.lines?.some((line) => line.includes("暂不买") && avoidActionCodes.some((code) => line.includes(code))), "customer decision summary must translate chase-risk rankings into a customer-readable avoid line");
assert(rankingBoard.customerDecisionSummary?.lines?.some((line) => line.includes("先补证据") && dataActionCodes.some((code) => line.includes(code))), "customer decision summary must translate data blockers into a customer-readable evidence line");
...
assert(adminSource.includes("compactRunnerConsoleText") && adminSource.includes("data-runner-view-target"), "admin virtual run console cards must stay inside runner sub-entries instead of jumping users into long report pages");
assert(/runner-workspace-view\[data-runner-view="control"\]\.active[\s\S]{0,220}overflow:\s*hidden/.test(adminStyleSource) && /run-console-grid[\s\S]{0,220}overflow:\s*auto/.test(adminStyleSource), "admin virtual run control view must bound summary cards inside the runner terminal");
...
assert(adminStyleSource.includes("portfolio-deployment-status") && adminStyleSource.includes("portfolio-deployment-status small") && adminStyleSource.includes("portfolio-rail-deployment"), "admin portfolio deployment status must be styled as a compact 1Panel-style status cell");
...
assert(/@media \(min-width:\s*861px\)[\s\S]*portfolio-workspace-group\.active\s*\{[\s\S]{0,180}display:\s*flex[\s\S]{0,180}overflow-x:\s*auto/.test(adminStyleSource), "admin portfolio desktop secondary entries must be horizontally scannable like a trading terminal toolbar");
...
const mainForceNewsPlaybook = manager.buildThemeMainForcePlaybook([mainForceNewsTheme], mainForceNewsBoards);
const mainForceProofDb = {
  ...
  runs: [{
    ...
    type: "market",
    status: "completed",
    marketSnapshot: { fetchedAt: freshThemeRefreshAt, themeLeaderboards: mainForceNewsBoards, themeMainForcePlaybook: mainForceNewsPlaybook }
  }]
};
...
assert(
  mainForceAbilityLane?.proofItems?.some((item) =>
    /作战图/.test(item.tag)
    && /人形机器人/.test(item.title)
    && /三花智控|载体锚点|主力证据/.test(`${item.detail} ${item.nextStep} ${(item.evidence || []).join(" ")}`)
  ),
  "main-force ability lane must prefer main-force playbook proof with carrier anchors over generic leaderboard slogans"
);
const staleMainForceProofPerformance = manager.buildPortfolioManagerPerformanceStats({
  ...
  runs: [{
    ...
    type: "market",
    status: "completed",
    marketSnapshot: { fetchedAt: "2000-01-01T09:30:00.000Z", themeLeaderboards: mainForceNewsBoards, themeMainForcePlaybook: mainForceNewsPlaybook }
  }]
});
...
assert(
  !staleMainForceAbilityLane?.proofItems?.some((item) =>
    /主力进场|题材预热|低位轮动|作战图/.test(String(item.tag || ""))
    && /人形机器人/.test(`${item.title} ${item.detail} ${(item.evidence || []).join(" ")}`)
  ),
  "manager ability proof must not show stale theme leaderboards or stale playbooks as current main-force evidence"
);
...
const themeNoFreshnessTraceQuality = manager.evaluateFundAnswerQuality({
...
assert(themeNoFreshnessTraceQuality.issues.includes("missing_theme_news_logic_explanation"), "quality gate must reject theme answers that omit the fresh news source or time trace");
const themeGenericWaitQuality = manager.evaluateFundAnswerQuality({
