...
assert(/@media \(min-width:\s*861px\)[\s\S]{0,900}body\[data-active-tab="portfolio"\] \.main[\s\S]{0,260}padding:\s*16px[\s\S]{0,900}\.portfolio-command-panel[\s\S]{0,360}max-height:\s*108px[\s\S]{0,2200}\.portfolio-workspace-switcher small[\s\S]{0,120}display:\s*none/.test(adminStyleSource), "admin portfolio desktop layout must compress the command header and terminal rail so the workspace is not a long page");
